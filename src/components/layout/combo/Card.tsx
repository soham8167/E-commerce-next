  ...
  return (
    <div className="flex flex-wrap gap-8 justify-center m-5">
      {products.map((item, index) => {
        // Disabled product (index === 1)
        if (index === 1) {
          return (
            <div
              key={item.id}
              onClick={() => alert("Product not available")}
              className="bg-[#FBF9F6] border-[#A59786] w-60 rounded-2xl p-4 shadow-md relative cursor-not-allowed opacity-80 hover:scale-105 transition"
            >
              {item.bestSeller && (
              ...
              <div className="text-[#E27148] flex justify-center text-2xl mt-3 font-semibold">
                <p>{item.msg}</p>
              </div>
            </div>
          );
