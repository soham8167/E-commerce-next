...
import type { fruitStoreState } from "../type/fruitState";

export const fruitVegStore = create<fruitStoreState>((set) => ({
  products: [
    {
      id: 1,
  ...
  loadFromServer: async () => {
    ...
    try {
      ...
      const fruits = items.filter((p: any) => p.category === "fruits");
      set((state) => ({
        products: [
          ...state.products,
          ...fruits
            .filter(
              (f: any) =>
                !state.products.find(
                  (s: any) => String(s.id) === String(f._id),
                ),
            )
            .map((f: any) => ({
              id: f._id,
